...
namespace Bio.IO.SAM
{
    ...
    {
        public SAMOptionalField()
        { }
        public SAMOptionalField(string tag, object value, char valueType)
        {
            this.Tag = tag;
            this.Value = value.ToString();
            if (!charToString.ContainsKey(valueType))
            {
                throw new ArgumentException(valueType.ToString() + " is not a valid SAM Optional field value type", "valuetype");
            }
            this.VType = charToString[valueType];
        }
        ...
        /// <summary>
        /// The types below are acceptable for BAM but not SAM, so are mapped to integer types
        /// </summary>
        //private static char[] BAMFiledToConvertToInteger = "`cCsSiI".ToCharArray();

        public static Dictionary<char,string> charToString= new Dictionary<char,string>() { {'c',"i"},{'C',"i"},{'s',"i"},{'S',"i"},{'i',"i"},{'I',"i"},
                                                                                            {'A',"A"},{'f',"f"},{'Z',"Z"},{'H',"H"},{'B',"B"}};
        ...
        public string VType
        {
            ...
            set
            {
                ...
                this.valueType = charToString[value[0]];
            }
        }

        public void SetValidatedVType(string value)
        {
        ...
        private static string IsValidVType(string vtype)
        {
            ...
            else if (charToString.ContainsKey(vtype[0]))
            {
                return String.Empty;
            }
            else
