...
namespace Bio.IO.Newick
{
    ...
    public class NewickParser : IPhylogeneticTreeParser, IDisposable
    {
        ...
        /// If the string "[&&NHX" is not present, throws an error 
        /// </summary>
        /// <returns></returns>
        private Dictionary<string, string> GetExtendedMetaData()
        {
            ...
            {
                throw new Exception("Was expecting a New Hampshire extended field collection, but found parser error.  Bad string is: " + str); 
            }
            ...
            foreach (var g in groups)
            {
                ...
                if(sub.Length!=2)
                {
                    throw new Exception("Could not split NHX metadata key/value pair: "+g);
                }
                result[sub[0]] = sub[1];
            }
            return result;
