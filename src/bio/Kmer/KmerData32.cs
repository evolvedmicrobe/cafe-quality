...
namespace Bio.Algorithms.Kmer
{
    ...
    /// Holds the KmerData.
    /// </summary>
    public struct KmerData32 :IEquatable<KmerData32>
    {
        ...
        public static KmerData32[] GetKmers(ISequence sequence, int kmerLength)
        {
            ...
            if (count < kmerLength || kmerLength > MAX_KMER_LENGTH)
            {
                throw new ArgumentException("Invalid kmerLength or sequence length.  Kmers must be between " + MIN_KMER_LENGTH + " and " + MAX_KMER_LENGTH +" and the read must be larger than the kmer length", "kmerLength");
            }
        ...
        {
            //Palindromes only possible with RCed sequences.  
            if (kmerLength % 2 == 0)
        ...
        private static ulong GetReverseComplement(ulong kmer, int kmerLength)
        {
            ...
            ulong reverse = 0;
            ulong mask = (ulong)((ulong)1 << (kmerLength*2)) - 1;
            ulong bits = 0;
            for (int index = 0; index < kmerLength; index++)
            {
                 ...
                 kmer = kmer >> 2;
                 // Just reverse order
                 reverse = (reverse << 2) + bits;
            }
            //now get the compliment and exclude bits out of range
            ...
            //    }
            //}
            //if(rev1!=reverse){throw new Exception("Yowza!!!");}
            //return reverse;
        }
        ...
        public bool Equals(KmerData32 obj)
        {
            return KmerData == obj.KmerData;
        }
        public static bool operator ==(KmerData32 self, KmerData32 other)
        {
