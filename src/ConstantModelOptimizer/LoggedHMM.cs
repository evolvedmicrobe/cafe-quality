...
namespace ConstantModelOptimizer
{
	...
	/// </summary>
	public sealed class LoggedHMM : PairHMM
	{		
		public LoggedHMM(bool doExactLog10)
		{
        }
		...
		private void initializeProbabilities(byte[] insertionGOP, byte[] deletionGOP, byte[] overallGCP)
		{
			for (int i = 0; i < insertionGOP.Length; i++)
			{
				...
				transition[i + 1][QUALITY] = QualityUtils.qualToProbLog10((byte) qualIndexGOP);
				transition[i + 1][OVERALL_GCP] = QualityUtils.qualToProbLog10(overallGCP[i]);
                transition[i + 1][INSERTION_GOP_ERROR] = QualityUtils.qualToErrorProbLog10(insertionGOP[i]);
				transition[i + 1][OVERALL_GCP_ERROR] = QualityUtils.qualToErrorProbLog10(overallGCP[i]);
				transition[i + 1][DELETION_GOP_ERROR] = QualityUtils.qualToErrorProbLog10(deletionGOP[i]);
                // note this is same value, not sure why.
				transition[i + 1][OVERALL_GCP_ERROR_2] = QualityUtils.qualToErrorProbLog10(overallGCP[i]);
			}
        ...
        #region TRANSITIONINDEXES
        public const int QUALITY = 0;
        public const int OVERALL_GCP = 1;
        public const int INSERTION_GOP_ERROR = 2;
        public const int OVERALL_GCP_ERROR = 3;
        public const int DELETION_GOP_ERROR = 4;
        public const int OVERALL_GCP_ERROR_2 = 5;
        #endregion
		...
		private void updateCell(int indI, int indJ, double prior, double[] transition)
		{
			...
			matchMatrix[indI][indJ] = prior + myLog10SumLog10(new double[]{matchMatrix[indI - 1][indJ - 1] + transition[QUALITY], insertionMatrix[indI - 1][indJ - 1] + transition[OVERALL_GCP], deletionMatrix[indI - 1][indJ - 1] + transition[OVERALL_GCP]});
