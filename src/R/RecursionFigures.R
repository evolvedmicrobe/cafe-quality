...
# Now for the Merge
pdf("Merge.pdf", width=pdf_width, height= pdf_height)
grid.newpage()
mkBase()
fillRect(oi,oj,"blue")
fillRect(oi-1,oj-2,"red")
addText(oi-1,oj-2,expression(alpha["i-1,j-2"]))
drawArrow(oi-1,oj-2,oi,oj)
colorLetter(oi-1,"read")
colorLetter(oj-1,"tpl","red")
colorLetter(oj-2,"tpl","red")
drawTextArrow("read_4","tpl_4")
drawTextArrow("read_4","tpl_3")
dev.off()

pdf("Everything.pdf", width=pdf_width, height= pdf_height)
grid.newpage()
mkBase(TRUE)
fillRect(oi,oj,"blue")
fillRect(oi-1,oj-2,"red")
