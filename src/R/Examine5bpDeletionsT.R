...
d = read.csv("homopolymerDeepDive5bpLongT.csv")
head(d)
nrow(d)
...
v[v$Correct==FALSE,"Zmw"]=v[v$Correct==FALSE,"Zmw"]/sum(v[v$Correct==FALSE,"Zmw"])
v[v$Correct==TRUE,"Zmw"]=v[v$Correct==TRUE,"Zmw"]/sum(v[v$Correct==TRUE,"Zmw"])
ggplot(v,aes(x=HPSizeGroup,y=Zmw,group=Correct,fill=Correct))+geom_bar(stat="identity",position=position_dodge())+theme_classic(base_size=16)+labs(x="Indel Size Relative to Correct Read",y="Percentage of Subreads")
...
tmp = rbind(set1,set2)
ggplot(tmp,aes(x=HPSizeGroup,fill=HPSize))+geom_density(aes(y=..density..),alpha=.8)+labs(x="Read Bias Score", title = "Read Bias by CCS Outcome")+theme_bw(base_size=16)+geom_vline(xintercept=0.33)

#Count Fixes
sum(set1$HPSizeGroup < .33)/nrow(set1)
...
head(v)
ggplot(v,aes(x=HomopolymerLength,fill=ReverseComplementedOriginally))+geom_density(alpha=.5)+labs(x = "Percentage of SubReads with Deletion", title=" Effect of reading T instead of A (A= Reverse Complement)" )+theme_bw(base_size=12)
