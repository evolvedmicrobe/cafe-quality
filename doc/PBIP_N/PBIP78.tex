...
\Abstract{ The PacBio consensus calling framework is complicated by the presence of deletion tag values attached to certain basepairs.  This PBIP reviews how these values are generated and evaluates from a principled and empirical perspective why they are undesirable.  It concludes that the CCS algorithm could likely benefit from richer deletion QV values, but that the DelTag value is likely to only complicate matters at present. }
